...
#             print type(boid),type(userId),type(sessionid)
            result1 = u"获取nonosession的msg为："+resp.json().get('msg')+"USERID="+userId+"SESSIONID="+sessionid
            result += result1
            ...
            result2 = u"执行任务计划响应结果为：--####--"+resp.text
            result += result2
            count += 1
        
        return result + u'实际共执行%s 次' % count 
    else:
        return u"出现错误,未查询到可以操作的数据！"
