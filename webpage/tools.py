...
def moneyper(money,qs,lv):
    ...
    return u'每期： %.2f   总额： %.2f ，每期利息为：%.2f' % (mq,mq*qs,(money*lv/100)/12)
...
def JY_moneyper(money,qs,lv,kt):
    ...
    return u'前%s期： %.2f ，后%s期： %.2f，总额为 %.2f' % (qs[0],lx,qs[1],bj+lx,sum_money) + baoli_res
...
def baoli(kt,lv,qs,money):
    ...
    if (lv+ts) < 11.88:
        ...
        if lv > 11.88 and kt != 0:
            ...
            bl =0 
            ts_money = 0
    return u" ,保理服务费为：%.2f，商户贴息费用为：%.2f，打款金额为：%.2f，消金账户的贴息利率:%.2f,商户贴息的年化率:%.2f" % (bl,ts_money,money-bl-ts_money,xjtxll,ts)
